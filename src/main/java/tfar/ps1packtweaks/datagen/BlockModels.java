...
public class BlockModels extends BlockModelProvider {
    ...
    protected void registerModels() {
        ...
        trapdoorModel("palm_trapdoor", PS1PackTweaks.id("block/palm_trapdoor"));

        tree("baobab");

        cubeTop("burrow", new ResourceLocation("block/sand"), PS1PackTweaks.id("block/burrow"));
    ...
    private void tree(String path) {
        ...
        cross(path + "_sapling", PS1PackTweaks.id("block/" + path + "_sapling"));
        singleTexture(path + "_leaves", new ResourceLocation("block/leaves"), "all", PS1PackTweaks.id("block/" + path + "_leaves"));
        cubeColumn(log, PS1PackTweaks.id("block/" + log), PS1PackTweaks.id("block/" + log + "_top"));
        cubeColumn(path + "_wood", PS1PackTweaks.id("block/" + log), PS1PackTweaks.id("block/" + log));
        strippedModels(path, PS1PackTweaks.id("block/" + log));
    }

    private void strippedModels(String path, ResourceLocation texture) {
        cubeColumn(path + "_log", texture, PS1PackTweaks.id(texture.getPath() + "_top"));
        cubeColumnHorizontal(path + "_log_horizontal", texture, PS1PackTweaks.id(texture.getPath() + "_top"));
    ...
    private void doorModel(String path) {
        ResourceLocation bottom = PS1PackTweaks.id("block/" + path + "_bottom");
        ResourceLocation top = PS1PackTweaks.id("block/" + path + "_top");
