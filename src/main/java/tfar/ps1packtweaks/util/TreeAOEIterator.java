...
//taken from https://github.com/SlimeKnights/TinkersConstruct/blob/1.20.1/src/main/java/slimeknights/tconstruct/library/tools/definition/module/aoe/TreeAOEIterator.java
    ...
    public record TreeAOEIterator(int width, int depth) implements AreaOfEffectIterator.Loadable {
        ...
        @Override
        public Iterable<BlockPos> getBlocks(UseOnContext context, BlockState state, AOEMatchType matchType) {
            int expanded = 0;
            return calculate(context.getLevel(),context.getClickedPos(), state, width, depth);
        }
         ...
         */
        public static Iterable<BlockPos> calculate(Level level, BlockPos origin, BlockState state, int extraWidth, int extraDepth) {
            return calculate(level, origin, state1 ->  state1.is(state.getBlock()), extraWidth, extraDepth);
        }
         ...
         * @return  Correct iterator for the targeted block
         */
        public static Iterable<BlockPos> calculate(Level level, BlockPos origin, Predicate<BlockState> state, int extraWidth, int extraDepth) {
            Direction depthDir;
            Direction widthDir;
            ...
            return () -> new TreeIterator(level, state, origin, widthDir, extraWidth, depthDir, extraDepth);
        }
        ...
        public static class TreeIterator extends AbstractIterator<BlockPos> {
            ...
            public TreeIterator(Level world, Predicate<BlockState> filter, BlockPos origin, Direction widthDir, int extraWidth, Direction depthDir, int extraDepth) {
                this.world = world;
                this.filter = filter;
            ...
            private boolean isValidBlock(BlockPos pos) {
                return filter.test(world.getBlockState(pos));
            }
