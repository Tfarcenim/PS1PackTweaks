...
public class TweakedJigsawPlacement {
    ...
    public static Optional<PieceGenerator<JigsawConfiguration>> addPieces(PieceGeneratorSupplier.Context<JigsawConfiguration> pContext,
                                                                          JigsawPlacement.PieceFactory pFactory,
                                                                          BlockPos pPos, boolean doExpansionHack,
        ...
        if (structurepoolelement == EmptyPoolElement.INSTANCE) {
            ...
            PoolElementStructurePiece structurePiece = pFactory.create(structuremanager, structurepoolelement, pPos,
                    structurepoolelement.getGroundLevelDelta(), rotation, structurepoolelement.getBoundingBox(structuremanager, pPos, rotation));
            ...
            if (!predicate.test(chunkgenerator.getNoiseBiome(QuartPos.fromBlock(i), QuartPos.fromBlock(k), QuartPos.fromBlock(j)))) {
                ...
                return Optional.of((p_210282_, p_210283_) -> {
                    ...
                    if (jigsawconfiguration.maxDepth() > 0) {
                        ...
                        Placer placer = new Placer(registry, jigsawconfiguration.maxDepth(), pFactory, chunkgenerator, structuremanager, list, worldgenrandom);
                        placer.placing.addLast(new PieceState(structurePiece, new MutableObject<>(Shapes.join(Shapes.create(aabb), Shapes.create(AABB.of(boundingbox)), BooleanOp.ONLY_FIRST)), 0));

                        while (!placer.placing.isEmpty()) {
                            PieceState pieceState = placer.placing.removeFirst();
                            placer.tryPlacingChildren(pieceState.piece, pieceState.free, pieceState.depth, doExpansionHack, levelheightaccessor);
                        }
    ...
    static final class Placer {
        ...
        void tryPlacingChildren(PoolElementStructurePiece piece, MutableObject<VoxelShape> p_210335_, int depth, boolean doExpansionHack, LevelHeightAccessor pLevelHeightAccessor) {
            ...
            for (StructureTemplate.StructureBlockInfo structuretemplate$structureblockinfo : structurepoolelement.getShuffledJigsawBlocks(this.structureManager, blockpos, rotation, this.random)) {
                ...
                if (optional.isPresent() && (optional.get().size() != 0 || Objects.equals(resourcelocation, Pools.EMPTY.location()))) {
                    ...
                    Optional<StructureTemplatePool> optional1 = this.pools.getOptional(fallback);
                    if (optional1.isPresent() && (optional1.get().size() != 0 || Objects.equals(fallback, Pools.EMPTY.location()))) {
                                ...
                                for (StructureTemplate.StructureBlockInfo structuretemplate$structureblockinfo1 : list1) {
                                    if (JigsawBlock.canAttach(structuretemplate$structureblockinfo, structuretemplate$structureblockinfo1)) {
                                        ...
                                        if (flag && flag2) {
                                            ...
                                            if (k == -1) {
                                                k = this.chunkGenerator.getFirstFreeHeight(blockpos1.getX(), blockpos1.getZ(), Heightmap.Types.WORLD_SURFACE_WG, pLevelHeightAccessor);
                                            }
                        ...
                        LOGGER.warn("Empty or non-existent fallback pool: {}", fallback);
                    }
                } else {
